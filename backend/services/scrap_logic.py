...
def run_scrap_and_save(query, num_pages, progress_ref=None):
    ...
    logger.info(f"Iniciando scraping para query='{query}', num_pages={num_pages}")
    ...
    for page_num, url in enumerate(urls, 1):
        ...
        logger.info(f"Productos encontrados en página {page_num}: {len(products)}")
            ...
            try:
                ...
                if link:
                    try:
                        ...
                        logger.error(f"Error al scrapear detalles del producto {link}: {e}")
                ...
                logger.error(f"Error al procesar producto en página {page_num}: {e}")
        time.sleep(2)
    logger.info(f"Total productos extraídos: {len(data)}")
    df = pd.DataFrame(data)
    # Deduplicación: elimina duplicados por 'title' y 'seller'
    df = df.drop_duplicates(subset=["title", "seller"])
    logger.info(f"Total productos tras deduplicar: {len(df)}")
    try:
        ...
        logger.info(f"Datos guardados en la base de datos. Registros insertados: {len(df)}")
    except Exception as e:
